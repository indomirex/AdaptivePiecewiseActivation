...
class VolatilityForecaster:
    ...
    def forecast_next_month(self, days=20):
        ...
        last_window = self.data.tail(30).copy()  # Increased window size for safety
        projections = []
        ...
        for i in range(days):
            ...
            features = pd.DataFrame({
                'ret_1': [last_window['Close'].iloc[-1] / last_window['Close'].iloc[-2] - 1 if len(last_window) >= 2 else 0],
    ...
    def forecast_test_set(self):
        """Project returns for the test set"""
        if not self.model:
            raise ValueError("Model not initialized. Train or load model first")
        ...
        test_preds = self.model.forward(X_test.values)
    
        # Create dataframe with actual and predicted values
        test_df = pd.DataFrame({
            'date': X_test.index,
            'actual_close': self.data.loc[X_test.index, 'Close'],
            'predicted_close': test_preds.squeeze()  # Use squeeze to remove extra dimension
        })
    ...
    def run(self):
        ...
        # Generate predictions for test set
        test_df = self.forecast_test_set()
        ...
        plt.plot(self.data.index, self.data['Close'], label='Actual Close Price')
        plt.plot(test_df['date'], test_df['predicted_close'], label='Predicted Close Price')
        plt.xlabel('Date')
        ...
        csv_path = f"{args.ticker}_predictions.csv"
